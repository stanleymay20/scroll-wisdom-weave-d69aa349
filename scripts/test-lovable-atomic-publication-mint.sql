...
DECLARE
  ...
  v_book_current uuid;
BEGIN
  -- Only the trusted backend may mint a verified publication.
  ...
  -- Design belongs to the certified publication state and must affect the hash.
  ...
  IF v_scope_hash IS NULL OR v_scope_hash !~ '^[0-9a-f]{64}$' THEN
  ...
  -- non_fiction does not require chapter evidence; all required whole-book
  -- gates are bound to this exact current manuscript/design hash.
  ...
  -- The mint rechecks authoritative state inside its own transaction. A
  -- manuscript change after review must reject the stale frozen scope and
  -- leave no Publication behind.
  UPDATE public.books
  ...
  -- Restore the exact reviewed state; the old attestations become current
  -- again because they are hash-bound to that state.
  UPDATE public.books
  ...
  -- A same-state retry may vary only volatile edition/frozen-at fields and
  -- must return the already-created publication.
  ...
  -- Immutable Publication snapshot itself.
  BEGIN
  ...
  -- Certified live manuscript/design consumed by readers must not drift.
  BEGIN
    UPDATE public.books
  ...
  -- Visibility/operational metadata is deliberately outside certified
  -- manuscript identity and must remain mutable.
  UPDATE public.books
