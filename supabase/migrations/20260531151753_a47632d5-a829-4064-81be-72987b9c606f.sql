-- Historical account-specific entitlement grant.
-- Apply it only when the intended auth identity exists. Clean installs must not
-- create fake auth records or relax foreign-key constraints to satisfy a
-- production-only account grant.
DO $$
DECLARE
  uid CONSTANT uuid := '607b86cf-c9cd-4ce3-bf4a-e60e6da09fcf'::uuid;
BEGIN
