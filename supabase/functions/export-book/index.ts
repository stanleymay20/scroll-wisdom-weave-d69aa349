...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-scroll-export-response, x-supabase-client-platform, x-supabase-client-platform-version, x-supabase-client-runtime, x-supabase-client-runtime-version",
};
      ...
      if (structuredMatch) {
        ...
        if (block) {
          ...
          for (let cli = 0; cli < structCodeLines.length; cli++) {
            const wrapped = wrapMonospaceLine((structCodeLines[cli] || "").replace(/\t/g, "  "), courier, 8, textWidth);
            for (const wl of wrapped) {
              if (y < textBottom + 12) {
                addRunningHeader(page, pageNumber, pageNumber % 2 === 1);
                page = pdfDoc.addPage([pageWidth, pageHeight]);
          ...
          if (block.output) {
            ...
            for (let oli = 0; oli < outLines.length; oli++) {
              const wrappedOut = wrapMonospaceLine((outLines[oli] || "").replace(/\t/g, "  "), courier, 8, textWidth);
              for (const wl of wrappedOut) {
                if (y < textBottom + 12) {
                  addRunningHeader(page, pageNumber, pageNumber % 2 === 1);
                  page = pdfDoc.addPage([pageWidth, pageHeight]);
...
async function generateEPUB(
  ...
  await zipWriter.add("mimetype", new zip.TextReader("application/epub+zip"), { level: 0, extendedTimestamp: false, ntfsTimestamp: false });
