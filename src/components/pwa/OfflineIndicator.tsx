...
export const OfflineIndicator = forwardRef<HTMLDivElement>(function OfflineIndicator(_, ref) {
  ...
  // Only show when we're truly offline (showOffline is true AND isOnline is false)
  // Never show false positives
  const shouldShow = showOffline && !isOnline;
  ...
  return (
    <AnimatePresence>
      {shouldShow && (
        ...
        >
          <div className="py-2.5 px-4 text-center text-sm font-medium shadow-md bg-amber-500/10 text-amber-700 dark:text-amber-400 border-b border-amber-500/20">
