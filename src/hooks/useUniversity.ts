...
export function useUniversity() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const firstError = [
        settingsRes, personRes, peopleRes, programmeRes, courseRes, termRes, cohortRes,
        offeringRes, enrolmentRes, assignmentRes, gradebookRes, transcriptRes, outcomeRes, moduleRes,
  ...
  const insert = useCallback(async (table: string, payload: Record<string, unknown>) => {
    ...
    const { data, error: insertError } = await db
      .from(table)
      .insert({ ...payload, organization_id: activeOrgId })
      .select()
      .single();
